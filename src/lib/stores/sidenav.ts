...
// Initialize the store with the current screen width and height
export const screenSize = writable<{ width: number; height: number }>();
...
screenSize.subscribe((value) => {
	...
	if (value && value.width <= 1020) {
		isSideNavOpen.update(() => false);
	} else {
		isSideNavOpen.update(() => true);
	}

	if (value && value.width <= 768) {
		document.body.classList.add('sidenav-mobile');
	} else {
		document.body.classList.remove('sidenav-mobile');
	}
});
...
export const sideNavItems = writable<SideNav[]>([
	...
	},
	{
		...
		disabled: false,
		path: '/dashboard/vendors-management'
	},
	{
		label: 'Riders Mgt.',
		icon: riderIcon,
	...
	},
	{
		...
		disabled: false,
		path: '/dashboard/ambulance-management'
	},
	{
		label: 'Order Mgt.',
		icon: orderIcon,
	...
	{
		label: 'Telemedicine Mgt.',
		icon: telemedicineIcon,
		title: 'Telemedicine Management',
		disabled: false,
		path: '/dashboard/telemedicine-management'
	},
	{
		label: 'Content Mgt.',
		icon: contentIcon,
	...
	},
	{
		...
		disabled: false,
		path: '/dashboard/master-marketing'
	},
	{
		...
		disabled: false,
		path: '/dashboard/internal-message'
	},
	{
		label: 'Settings',
		icon: settingsIcon,
...
selectedNavItem.subscribe((value) => {
	const sideNavIsOpen = get(isSideNavOpen);

	if (sideNavIsOpen && value) {
		isSideNavOpen.update(() => false);
	}
});